...
const createMockAudioBlob = (): Blob => {
  ...
  for (let i = 0; i < numSamples; i++) {
    ...
    const beat1 =
      ...
      Math.exp(-((time % (1 / beatFreq)) * 20));
    const beat2 =
      Math.sin(2 * Math.PI * (frequency * 1.5) * time) *
      Math.exp(-(((time + 0.1) % (1 / beatFreq)) * 15));
...
export const loadAorticWAVFile = (): Promise<Blob> => {
  const filePath =
    "/Users/liul31/gradathoners-ctohackathon2024/resources/sounds/aortic.WAV";
