...
const createCompleteRecordingBatch = async (
  ...
  console.log(`Completed recording batch ${batch.id} for Trevin Wadu with ${recordings.length} recordings`);
 ...
 */
const findPatientByNameOrUID = async (name: string, uid?: string): Promise<Patient | null> => {
  try {
...
export const seedMockDataIfEmpty = async (): Promise<boolean> => {
  try {
    ...
    if (existingPatients.length > 0) {
      console.log('Database already contains patient data, skipping mock data seeding');
      return false;
    }
